'use client';

import { useState } from 'react';
import { setAuth } from '@/lib/api';

const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api';

export default function LoginPage() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const handleGoogleLogin = () => {
    setLoading(true);
    // Server-side redirect flow → Google consent → callback with tokens
    window.location.href = `${API_URL}/auth/google/login`;
  };

  return (
    <div className="login-page">
      <div className="login-card">
        {/* Brand */}
        <div className="login-logo">
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
            <div style={{
              width: '56px', height: '56px',
              background: 'linear-gradient(135deg, #06b6d4, #8b5cf6)',
              borderRadius: '14px',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: '1.5rem', fontWeight: 800, color: 'white'
            }}>⚙️</div>
          </div>
          <h1>cnc.mayyanks.app</h1>
          <p>CNC Intelligence & Predictive Automation</p>
        </div>

        {/* Features */}
        <div style={{ margin: '1.5rem 0', padding: '1rem', background: 'rgba(34, 211, 238, 0.05)', borderRadius: '10px', border: '1px solid rgba(34, 211, 238, 0.1)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.75rem', color: '#94a3b8' }}>
            <span>🔮 Predictive Maintenance</span>
            <span>📊 Real-time Analytics</span>
            <span>🤖 AI Copilot</span>
            <span>🔧 G-code Optimization</span>
            <span>🌐 Digital Twin</span>
            <span>⚡ Fleet Intelligence</span>
          </div>
        </div>

        {error && <div className="login-error">{error}</div>}

        {/* Google Sign-In — Primary & Only Auth */}
        <button
          className="btn btn-google"
          onClick={handleGoogleLogin}
          disabled={loading}
          style={{ fontSize: '0.95rem', padding: '0.875rem 1.5rem' }}
        >
          <svg width="20" height="20" viewBox="0 0 24 24">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
          </svg>
          {loading ? 'Redirecting to Google...' : 'Sign in with Google'}
        </button>

        <p style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.7rem', color: '#475569' }}>
          By signing in, you agree to the cnc.mayyanks.app Terms of Service
        </p>

        <div style={{ textAlign: 'center', marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid var(--border)' }}>
          <p style={{ fontSize: '0.65rem', color: '#64748b', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
            Secured with Google OAuth 2.0
          </p>
        </div>
      </div>
    </div>
  );
}
